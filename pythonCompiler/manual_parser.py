...
class Parser:
    ...
    def consume(self, expected_type=None):
        ...
        if current_token[0] == 'T_NEWLINE':
            self.current_line += 1
            
        if expected_type and current_token[0] != expected_type:
            raise SyntaxError(f"Line {self.current_line}: Expected {expected_type} but found {current_token}")
    ...
    def program(self):
        """Parse program -> package_decl class_decl"""
        ...
        # Parse class declaration (one or more)
        while self.peek() == 'T_CLASS':
            class_node = self.class_decl()
            program_node.add_child(class_node)
